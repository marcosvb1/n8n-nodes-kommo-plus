...
import { IFormTransaction, RequestTransactionCreate } from '../types';
...
export async function execute(
	...
	const transactionsCollection = (await this.getNodeParameter('collection', 0)) as IFormTransaction;
	...
	const body = transactionsCollection.transaction
		.map((transaction): RequestTransactionCreate => {
	...
	const responseData = await apiRequest.call(this, requestMethod, endpoint, { add: body });
	return this.helpers.returnJsonArray(responseData);
}
